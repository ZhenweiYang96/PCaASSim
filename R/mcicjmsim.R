...
#' }
#' @param sens_fit biopsy sensitivity used to generate the underlying event times. This value can be chose from (0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9).
#' @param sens_sim biopsy sensitivity used to generate the biopsy time in the simulated dataset. The default value is the same as `sens_fit`.
...
mcicjmsim <- function(n = 1000, seed = 100,
  ...
  } else if (sched_sim == "yearly") {
    fv_idx <- seq(1, K, 4)
  } else if (sched_sim == "biyearly") {
    fv_idx <- seq(1, K, 8)
